/**
 * dsh-sync-plugin —— DeepSeek Harness 一键同步插件(开源版主入口)
 *
 * 安装(一行命令):
...
/** 列出本机全部会话(标题来自投影缓存,尽力而为) */
function sessionRows(home) {
  const out = [];
  let titles = {};
  try {
    const proj = JSON.parse(fs.readFileSync(path.join(home, 'storages', 'session_projcache.json'), 'utf8'));
    titles = proj?.tables?.sessions ?? {};
  } catch { /* 无缓存就只显示 id/时间 */ }
  ...
  for (const group of groups) {
    ...
    for (const id of ids) {
      ...
      const t = titles[id]?.rows?.title?.val;
      const title = typeof t === 'string' ? t : (typeof t?.title === 'string' ? t.title : '');
      out.push({ id, group, title, mtimeMs, sizeKB: Math.round(size / 1024) });
    }
  }
...
/** 删除会话:移入本地回收站 .dsh/.trash(可找回);清理归档标记 */
function deleteSession(home, id) {
  ...
  for (const group of groups) {
    ...
    const trashDir = path.join(home, '.trash', Date.now() + '-' + id);
  ...
  return { id, groups: moved, trash: path.join(home, '.trash') };
}
...
function apply(ctx, config = {}) {
  ...
  try {
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        const archived = archivedSet(home);
        const sessions = sessionRows(home).map((r) => ({ ...r, archived: archived.has(r.id) }));
  ...
  try {
    disposers.push(ctx.tools.register({
      name: 'session_list',
      description: '列出本机全部 DSH 会话(id、标题、最后活动时间、大小、是否已归档),按最近活动排序。用于在删除前定位会话。',
      parameters: {},
      output: toolOut,
      async execute() {
        const archived = archivedSet(home);
        ...
        return rows.slice(0, 80).map((r, i) =>
          `${i + 1}. ${r.title || '(无标题)'} | ${r.id} | ${new Date(r.mtimeMs).toLocaleString()} | ${r.sizeKB}KB${archived.has(r.id) ? ' | 已归档' : ''}`,
        ).join('\n');
      },
    }));
    disposers.push(ctx.tools.register({
      ...
      async execute(args) {
        ...
        return `已删除会话 ${r.id}(所在组: ${r.groups.join(', ')})。回收站: ${r.trash}。若它正在页面中打开,刷新页面后消失;下次点「同步」会把删除传播到其他电脑。`;
      },
    }));
  } catch (err) {
  ...
  log('info', `dsh-sync-plugin 已启动(mode=${cfg.mode || 'manual'}, home=${home}, 分支=${cfg.branch}, remote=${cfg.remote || '(未配置,仅本地快照)'}, 页面按钮=左下角「⟳ 同步」)`);
}
